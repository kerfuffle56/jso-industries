      ...
      <section className="relative h-screen min-h-[640px] flex items-end overflow-hidden bg-[#0d0d0d]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pb-20 md:pb-28 w-full">
          ...
          <p className="fade-up-2 text-[16px] md:text-[18px] text-[#888] font-light leading-relaxed max-w-lg mb-10">
            We acquire, entitle, and build our own projects. We bring that same developer perspective to yours.
          </p>
          <div className="fade-up-3 flex flex-wrap gap-4">
            ...
            <Link href="/contact" className="px-8 py-3.5 border border-[#444] text-white text-[12px] font-semibold tracking-[0.12em] uppercase hover:border-white transition-colors">
              Get in Touch
            </Link>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-[#e8e8e8]">
            {STATS.map((s) => (
              <div key={s.label} className="px-8 py-10">
                ...
                <p className="text-[11px] text-[#aaa] uppercase tracking-[0.12em]">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <p className="text-[16px] text-[#555] leading-relaxed mb-6">
                JSO Industries is a vertically integrated real estate development and general contracting firm based in Westchester County, NY. Founded by Jonathan Oronzio, a decade of field experience means we do not just plan projects — we execute them.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 md:py-36">
          ...
          <div className="border border-[#e0e0e0] bg-white">
            <div className="grid grid-cols-1 md:grid-cols-3">
              {AREAS.map((a, i) => (
                <div key={a.state} className={`p-8 md:p-10 border-b md:border-b-0 border-[#e8e8e8] last:border-b-0 ${i < 2 ? "md:border-r" : ""}`}>
                  <p className="text-[13px] font-semibold text-[#bf2a2a] mb-5">{a.state}</p>
                  <div className="flex flex-col gap-3">
                    {a.places.map((p) => <p key={p} className="text-[15px] text-[#555]">{p}</p>)}
                  </div>
                </div>
              ))}
            </div>
          </div>
          <p className="text-[12px] text-[#bbb] mt-6 font-mono">Licensed and fully insured in New York, Connecticut, and New Jersey.</p>
        </div>
      </section>
